...
def parse_method(row):
    ...
    parsed_row['department'] = str(row[1])
    return parsed_row
...
def write_to_sql(element):
    ...
    cursor = conn.cursor()
    sql = f"INSERT INTO {TABLE_NAME} (id, department) VALUES (?, ?)"
    values = (element['id'], element['department'])
...
def run():
    ...
    with beam.Pipeline() as p:
        ...
        lines = p | 'ReadFromGCS' >> beam.io.ReadFromText(f'gs://{BUCKET_NAME}/departamentos/*.csv')
